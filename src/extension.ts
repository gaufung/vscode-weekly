...
import { PostBlog } from "./postDataProvider"
...
function GetWebViewContent(filename: string) : string {
	...
	// //let userAgent = "curl/7.64.1"
	// let response = new SyncRequestClient().addHeader("User-Agent", userAgent).addHeader("Accept","application/vnd.github.v3.raw+json").get<string>("https://api.github.com/repos/ruanyf/weekly/contents/docs/"+filename)
	// //let response = new SyncRequestClient().addHeader("User-Agent", userAgent).get<string>("https://raw.githubusercontent.com/ruanyf/weekly/master/docs/"+filename);
	// // let encodeContent = JSON.parse(JSON.stringify(response))['content']
	// // var base = new Base64();

	// //let decodeContent = base.decode(encodeContent);
	// let decodeContent = JSON.stringify(response);
	let content = fetchRawContent(filename);
...
function fetchRawContent(filename:string): string {
	let userAgent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36"
	let url = "https://api.github.com/repos/ruanyf/weekly/contents/docs/"+filename
	let options : any= {
    	headers : {
